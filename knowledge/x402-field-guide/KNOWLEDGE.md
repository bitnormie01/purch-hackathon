...
**Why:** Prices on Purch are dynamic. The search endpoint returns a snapshot. By the time your agent calls the buy endpoint, the seller or system may have adjusted the price. There is no documented price-hold mechanism or cache window between search and buy — treat every search price as an estimate.
...
**Why:** Solana transactions are irreversible. There is no undo. If your agent wraps `fetchWithPay` in a generic retry loop, each attempt creates a new on-chain payment. The x402 client does not know about your outer retry logic. Based on the `@x402/fetch` source, there is no built-in deduplication — each call to `fetchWithPay` is an independent payment attempt. Do not rely on idempotency.
...
**Why:** The `downloadToken` is intentionally short-lived. It is a one-time-use or time-limited credential that proves the download is happening as part of the same session as the purchase. Based on the integration code pattern (which downloads immediately after buy with no delay), the expiry is likely in the range of 5–15 minutes. The working code in `auto-equip-agent/src/buyer.ts` treats it as potentially single-use and never delays between buy and download.
...
3. If the token has expired, there is likely no way to get a new one without a new purchase. There is no documented token refresh mechanism for a given `purchaseId`.
...
3. For pre-payment budget enforcement, you'd need to handle the 402 loop manually instead of using `wrapFetchWithPayment`. Based on the `@x402/fetch` package source, there is no `beforePayment` hook — the payment is fully automatic. The workaround is to probe the endpoint with plain `fetch` first (to get the 402 challenge price), validate the budget, and only then call `fetchWithPay`.
...
| **POST** | `/x402/buy` | Item price | Product details in body | Purchase confirmation (marketplace-specific response) |
